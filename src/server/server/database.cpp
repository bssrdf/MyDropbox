...
bool Server::ReturnSharedUsers( string finame , string uID){
    if ( !db.isOpen() ) {
        db.open();
    }
    QSqlQuery fetcher;
    ofstream f;
    f.open("RandFile.txt");
    string p1("SELECT USER FROM ");
    string p2(SHAREDTABLE);
    string p4(" WHERE OWNER='");
    string p5("' AND FILENAME='");
    string p6("';");
    string execCmd= p1 +p2 + p4+uID  + p5+finame + p6;
    cout << execCmd << "\n";
    fetcher.exec( execCmd.c_str() );
    f << "LOLFAIL\n";
    while ( fetcher.isSelect() && fetcher.next() ) {
        cout << fetcher.value(0).toString().toUtf8().constData()<< "\n";
        f << fetcher.value(0).toString().toUtf8().constData()<<endl;
    }
    db.close();
    f.close();
    return true;
