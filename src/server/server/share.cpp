...
bool Server::handleMyShare(){
    //You Need filename , userID;
    //ASSERT : cmd has been read.
    string cont(CONTINUE);
    conn.writeToSocket(cont);
    string fn , uid;
    conn.readFromSocket(fn);
    conn.writeToSocket(cont);
    conn.readFromSocket(uid);
    conn.writeToSocket(cont);
    ReturnSharedUsers(fn , uid); //Creates a file called "RandFile.txt".
    string _file = "RandFile.txt";
    conn.writeToSocket_file(_file);
    conn.readFromSocket(cont);
//    handleSync(user.userID);
    return true;
}
