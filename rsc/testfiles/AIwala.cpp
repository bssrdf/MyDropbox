...
#define notg(i,k) (-1*((k-1)*vertices + i )) 
#define g(i,k) ((k-1)*vertices + i )
//#define notg(i,k) (-1*((i-1)*subGraphs + k ))
//#define g(i,k) ((i-1)*subGraphs + k )
...
void initialize()
{
	...
	//variablenumber=vertices*vertices*subGraphs + edges*subGraphs;
	//clausenumber= 
	//cout<<"p cnf "<<variablenumber<<" "<<clausenumber<<endl;
}
	...
	for (int k=1; k<=subGraphs; k++)
		for (int l=1; l<=subGraphs; l++)
		{
			if(l!=k)
			{
				for (int i=1; i<=vertices; i++)
				{
					cout<<knbias<<" "<<notg(i,k)<<" "<<g(i,l)<<" 0"<<endl;
					cout<<-1*knbias<<" "<<g(i,k)<<" 0"<<endl;
					cout<<-1*knbias<<" "<<notg(i,l)<<" 0"<<endl;
					knbias++;
					// clauseCounter+=3;
					// clauseCounter4+=3;
				}
				//cout<<"clause"<<endl; //DEBUG 
				knbias-=vertices;
				for (int i=1;i<=vertices;i++)
				{
					cout<<knbias<<" ";
